...
public class ChefClientType extends HpCloudItemType {
    public static final String TYPE_LOCAL_ID = "chef_client";
    public static final String[] PAYLOAD_FIELDS = {"certificate", "orgname", "rev", "org_guid", "id", "clientname"};
    private static final Log LOG = LogFactory.getLog(ChefClientType.class);

    public ChefClientType() {
        super(TYPE_LOCAL_ID);
        ...
        for (String attName : PAYLOAD_FIELDS) {
            Attribute attribute;
            try {
                attribute = new Attribute.Builder(attName).name(attName).visible(true).plottable(false).build();
                ...
                LOG.error("Problem creating ChefClientType", e);
            }
        }
    }
