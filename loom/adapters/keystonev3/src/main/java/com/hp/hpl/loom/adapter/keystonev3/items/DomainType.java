...
public class DomainType extends SeparableItemType {
    private static final Log LOG = LogFactory.getLog(DomainType.class);
    public static final String TYPE_LOCAL_ID = "domain";

    public DomainType(final Provider provider) {
        super(TYPE_LOCAL_ID);
        ...
        try {
            ...
            this.addAttributes(enabled, project, user);
            this.addOperations(DefaultOperations.SORT_BY.toString(),
                    new OperationBuilder().add(enabled, project, user).build());
            this.addOperations(DefaultOperations.GROUP_BY.toString(),
                    new OperationBuilder().add(enabled, user, project).build());
        } catch (AttributeException e) {
            LOG.error("Problem constructing DomainType attributes: ", e);
        }
    }
