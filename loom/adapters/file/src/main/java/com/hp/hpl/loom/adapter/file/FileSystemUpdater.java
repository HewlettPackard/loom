...
public class FileSystemUpdater extends AggregationUpdater<FileItem, FileItemAttributes, File> {
    ...
    protected void setRelationships(final ConnectedItem fileItem, final File resource) {
        ...
        if (dir != null) {
            fileItem.setRelationshipWithType(this.adapter.getProvider(), Types.DIR_TYPE_LOCAL_ID, dir.getAbsolutePath(),
                    "contains");
        }

        List<File> dirs = FileUtils.getAllAncestors(FileUtils.getFileRoot(adapter), dir);
        for (File file : dirs) {
            fileItem.setRelationshipWithType(this.adapter.getProvider(), Types.DIR_TYPE_LOCAL_ID,
                    file.getAbsolutePath(), "ancestor");
        }
    }
}
