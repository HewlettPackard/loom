...
define(['lodash', 'jquery'], function (_, $) {
  ...
  var template = require('./FiberActionPanel.html');
  ...
  /**
   * FiberActionPanel displays the fiber action form to allow the user to perform the action
   *
   * @class FiberActionPanel
   * @namespace views
   * @module  weaver
   * @submodule views
   * @extends BaseView
   * @constructor
   */
  var FiberActionPanel = BaseView.extend({
     ...
     * @final
     */
    constructorName: 'LOOM_FiberActionPanel',

    className: "mas-fiberActionPanel",
    ...
    events: {
      ...
      'click .mas-dialog--back': function(event) {
        ...
        this.EventBus.trigger('fiber:action:cancel');
      }
    },
     ...
     * options.fiber - the fiber which the action is performing on
     * @method initialize
     */
    initialize: function (options) {
      BaseView.prototype.initialize.apply(this, arguments);
      this.model = options.fiber;
      this.render();
    },

    /**
     * Renders the various components into the tabbed side menu view
     * @returns {FiberActionPanel}
     * @chainable
     */
    render: function() {
      this.$('.mas-fiberActionPanel--title').html(this.options.action.get('name'));
      this.$('.mas-fiberActionPanel--description').html(this.options.action.get('description'));
      _.forEach(this.options.action.get('params'), function (param) {
        this.$('.mas-fiberActionPanel--controls').append(this._createControls(param));
      }, this);
      return this;
    },
    ...
    executeModelAction: function(inputs) {
      this.EventBus.trigger('fiber:action:server:send', {
        element: this.model,
        values: inputs,
        action: this.options.action
      });
      this.options.action.execute(this.options.fiber, inputs).then(
    ...
    onSuccess: function (response) {
      this.EventBus.trigger('fiber:action:server:send:success', {
        element: this.model,
        response: response,
        action: this.options.action
      });
    ...
    onFail: function (response) {
      this.EventBus.trigger('fiber:action:server:send:fail', {
        element: this.model,
        response: response,
        action: this.options.action
      });
