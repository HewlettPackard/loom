...
define(function (require) {
  ...
  var template = require('weaver/views/PropertySelectorSideMenu.html');
   ...
   * @constructor
   */
  var PropertySelectorSideMenu = BaseView.extend({
     ...
     * @final
     */
    constructorName: 'LOOM_PropertySelectorSideMeu',
    ...
    events: {
      ...
      'click .mas-threadConfiguration--metric': '_handleClick',
      'click .mas-threadConfiguration--quicksort': '_handleClick',
    ...
    select: function (property, silent) {
      ...
      if (!silent) {
        ...
        this.EventBus.trigger('thread:metric:selected', {
          thread: this.thread,
    ...
    unselect: function (property) {
      if (this.options.multiple) {
        ...
        if (removed.length) {
          ...
          this.trigger('change:selection', this.options.selection);
          this.EventBus.trigger('thread:metric:unselected', {
            thread: this.thread,
            properties: this.model,
            metric: property
          });
    ...
    _createBlankElement: function () {
      ...
      var extraClasses = this.options.additionalItemClass ? ' ' + this.options.additionalItemClass : '';
      li.className = 'mas-sideMenuListItem mas-propertySelector--property mas-property mas-property-blank mas-property-selected' + extraClasses;
    ...
    _createPropertyElement: function (property, propertyID) {
      if (property) {
        var itemClasses = [
          'mas-sideMenuListItem',
          'mas-propertySelector--' + propertyID
        ];
        if (this.options.additionalItemClass) {
          itemClasses.push(this.options.additionalItemClass);
        }
        ...
        if (this.options.propertyView) {
          ...
          view.$el.addClass(itemClasses.join(' '));
          view.$el.data('property', propertyID);
          return view.el;
        }
        var li = document.createElement('li');
        itemClasses.push('mas-property');
  ...
  return PropertySelectorSideMenu;
});
