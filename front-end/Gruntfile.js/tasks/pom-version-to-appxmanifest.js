...
var task = module.exports = function (grunt) {
  ...
  grunt.registerMultiTask('pom-version-to-appxmanifest', 'Pulls version number from a Maven POM file to insert it in a Windows8 .appxmanifest', function () {

    var options = this.options(defaults);
    var done = this.async();
    var pomVersion = require('../lib/pom-version');

    var updateVersion = require('../lib/update-appxmanifest-version');
    pomVersion(options.pom, function (err, version) {
      ...
      require('async').each(options.src, function (path, callback) {
        updateVersion(path, version, callback);
      }, done);
    });
  });
};
